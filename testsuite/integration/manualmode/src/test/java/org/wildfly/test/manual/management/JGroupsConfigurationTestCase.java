...
public class JGroupsConfigurationTestCase {
    ...
    public void testTcpNio2Attributes() throws Exception {
        try {
            ...
            cli.sendLine("/subsystem=jgroups/stack=tcp/transport=TCP_NIO2:read-attribute(name=server-ssl-context)");
            Assert.assertThat(cli.readOutput(), containsString("undefined"));
            cli.sendLine("/subsystem=jgroups/stack=tcp/transport=TCP_NIO2:read-attribute(name=client-ssl-context)");
            Assert.assertThat(cli.readOutput(), containsString("undefined"));
        } finally {
            cli.sendLine("batch");
