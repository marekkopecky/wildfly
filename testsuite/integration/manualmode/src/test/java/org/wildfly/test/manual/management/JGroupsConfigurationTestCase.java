...
public class JGroupsConfigurationTestCase {
    ...
    /**
     * Check that ssl context attributes are not available on TCP_NIO2 transport
     */
    @Test
    @InSequence(4)
    public void testTcpNio2Attributes() throws Exception {
        try {
            ...
            try {
                cli.sendLineForValidation("/subsystem=jgroups/stack=udp/transport=TCP_NIO2:read-attribute(name=server-ssl-context)");
                Assert.fail("Exception has been expected, but it has not been thrown");
            } catch (CommandLineException e) {
                Assert.assertThat("Wrong exception", e.getMessage(), containsString("WFLYCTL0201"));
            }
            try {
                cli.sendLineForValidation("/subsystem=jgroups/stack=udp/transport=TCP_NIO2:read-attribute(name=client-ssl-context)");
                Assert.fail("Exception has been expected, but it has not been thrown");
            } catch (CommandLineException e) {
                Assert.assertThat("Wrong exception", e.getMessage(), containsString("WFLYCTL0201"));
            }
