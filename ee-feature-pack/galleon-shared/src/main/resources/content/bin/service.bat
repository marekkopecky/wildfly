...
if /I "%IS_DOMAIN%" == "true" (
  ...
  if not "!CREDENTIALS!" == "" (
    ...
    set STOPPARAM="/c#!STOP_SCRIPT!#--controller=!CONTROLLER!#--connect#--command=/host=!DC_HOST!:shutdown"
  )
) else (
  ...
  if not "!CREDENTIALS!" == "" (
    ...
    set STOPPARAM="/c#!STOP_SCRIPT!#--controller=!CONTROLLER!#--connect#--command=:shutdown"
  )
)
